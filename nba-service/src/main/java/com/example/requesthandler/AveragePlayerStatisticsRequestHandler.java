...
public class AveragePlayerStatisticsRequestHandler implements HttpHandler {
  private static final Logger logger =
      LoggerFactory.getLogger(AveragePlayerStatisticsRequestHandler.class);
  ...
  public void handle(HttpExchange exchange) throws IOException {
    if (HttpMethod.GET.getValue().equals(exchange.getRequestMethod())) {
      try {
        ...
        AggregatedStatistics aggregatedStatistics = DbService.getInstance().getPlayerStatistics(id);
        exchange
            .getResponseHeaders()
            .set(
  ...
  private Integer getId(HttpExchange exchange) {
    ...
    if (StringUtil.isEmpty(playerId)) {
      throw new BadRequestException(Set.of(new Violation("playerId", "must not be null")));
    }
